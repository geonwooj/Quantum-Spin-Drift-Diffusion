...
def train_alt(flower_ds, car_ds, cfg: TrainConfig):
    ...
    drift = DriftA_NoGain(
        betas, DriftCfg(K=cfg.K, A=2.0, tau0=1e-4, lp_sigma=3.0, freeze_prototypes=False)
    )
...
def build_and_load_latest(model_dir, K=1000):
    ...
    # 4) 최신 가중치 로드
    wlist = sorted(glob.glob(os.path.join(WEIGHTS_DIR, "denoise_fn_step0050000.weights.h5")))
